...
class WrappedMinkUNet(nn.Module):
    def __init__(self, cfg):
        super().__init__()
        self.model = MinkUNet14(in_channels=3, out_channels=cfg.out_feat_dim)
        return

    def forward(self, data):
        batch_size, point_num, _ = data["point_clouds"].shape  # (B, N, 3)
        coor = data["coors"]  # (M, 4)
        feat = data["feats"]  # (M, 3)
        mink_input = ME.SparseTensor(feat, coordinates=coor)
        mink_output = self.model(mink_input).F
        local_feature = mink_output[data["quantize2original"]].view(
